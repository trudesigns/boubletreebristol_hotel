...
function CheckAuthentication()
{
	...
	// some kind of session validation here. Even something very simple as...las
...
//print_r($_COOKIE);exit;
        $cookie = $_COOKIE['ckfinder_access'];
        $ex = explode("~",$cookie);
         if( $ex[1]=== md5(date("Y-m-d"))){
            return true;
        } else {
            return false;
        } 
...
/* valid as of May 18, 2012 */
$config['LicenseName'] = 'Pita Group';
$config['LicenseKey'] = 'JSQU-MEA7-67D3-2SN7-PBYW-FP9E-T1N3';
...
//echo "HSHSHSHSHHSHS<br />";
//
//print_r($_COOKIE);exit;
$base_folder = "";
if (isset($_COOKIE["ckfinder_baseURL"])&& $_COOKIE["ckfinder_baseURL"]) {
   
        $base_folder = $_COOKIE['ckfinder_baseURL'];
    
}

//echo "BASE: ".var_dump($base_folder);exit;

if($base_folder != "")
{
	$baseUrl = '/assets/uploads/secure/'.$base_folder."/";
}
else
{
